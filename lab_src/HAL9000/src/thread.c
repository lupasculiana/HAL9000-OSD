    ...
    )
{
    ...
    LockInit(&m_threadSystemData.TotalThreadsLock);
}
...
#pragma warning(suppress:4152)
    ...
    *Thread = pThread;
    ...
    m_threadSystemData.TotalThreads++;

    LOG("Thread [ID=%d] was created with the name %s \n", pThread->Id, pThread->Name);
    return status;
}
    ...
    )
{
    ...
    LOG("Thread [ID=%d] has ended with the name %s \n", pThread->Id, pThread->Name);
    ...
    )
{
    ...
    __try
    {
        ...
        LockAcquire(&m_threadSystemData.TotalThreadsLock, &oldIntrState);
        m_threadSystemData.TotalThreads++;
        LockRelease(&m_threadSystemData.TotalThreadsLock, oldIntrState);
    }
    __finally
    {
        if (!SUCCEEDED(status))
